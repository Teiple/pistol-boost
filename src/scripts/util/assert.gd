...
static func in_rangef(val: float, min_inclusive: float, max_inclusive: float, context: String) -> void:
...
static func array_has_size(array: Array, expected_size: int, context := "") -> void:
	assert(
		array.size() == expected_size,
		MSG_FMT
		% [context, "Array should have size %d but it was %d" % [expected_size, array.size()]],
	)
...
static func of_type(value: Variant, type_code: int, context := "") -> void:
	assert(
		...
		MSG_FMT % [context, "This value should be of type %s" % type_string(type_code)],
	)


static func array_contains(array: Array, val: Variant, context := "") -> void:
...
static func positivei(val: int, context: String) -> void:
	assert(
		val > 0,
		MSG_FMT % [context, "This integer should be greater than zero but it was %d" % val],
	)
...
static func positivef(val: float, context: String) -> void:
	assert(
		val > 0,
		MSG_FMT % [context, "This float should be greater than zero but it was %d" % val],
	)
...
static func same_instance(val: Variant, other_val: Variant, context := "") -> void:
	assert(
		is_same(val, other_val),
		MSG_FMT % [context, "Values should reference the same instance"],
	)
...
static func not_equali(val: int, compare_to: int, context: String) -> void:
	assert(
		val != compare_to,
		MSG_FMT % [context, "This integer should not be equal to %d" % compare_to],
	)
...
static func greateri(val: int, compare_to: int, context: String) -> void:
	assert(
		val > compare_to,
		MSG_FMT
		% [context, "This integer should be greater than %d but it was %d" % [compare_to, val]],
	)
...
static func greaterf(val: float, compare_to: float, context: String) -> void:
	assert(
		val > compare_to,
		MSG_FMT
		% [context, "This float should be greater than %f but it was %f" % [compare_to, val]],
	)
...
static func lessf(val: float, compare_to: float, context: String) -> void:
	assert(
		val < compare_to,
		MSG_FMT % [context, "This float should be less than %f but it was %f" % [compare_to, val]],
	)
