...
static func lookat(node: Node3D, lookat_position: Vector3, local_axis: Vector3 = Vector3(0, 0, -1)) -> void:
	lookat_direction(node, (lookat_position - node.global_position).normalized(), local_axis)


static func lookat_direction(node: Node3D, direction: Vector3, local_axis: Vector3 = Vector3(0, 0, -1)) -> void:
